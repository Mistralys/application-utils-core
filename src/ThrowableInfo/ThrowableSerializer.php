...
class ThrowableSerializer
{
     ...
     */
    public static function serialize(ThrowableInfo $info)
    {
        $result = array(
    ...
    private static function createTypeException(string $keyName, string  $expectedType) : ConvertHelper_Exception
    {
        return new ConvertHelper_Exception(
            ...
            ThrowableInfo::ERROR_INVALID_SERIALIZED_DATA_TYPE
        );
    }
     ...
     * @throws ConvertHelper_Exception
     */
    private static function unserializePrevious(?array $previous) : ?ThrowableInfo
    {
        if(!empty($previous))
        {
            return ThrowableInfo::fromSerialized($previous);
        }
