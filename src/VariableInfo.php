...
class VariableInfo implements Interface_Optionable
{
    ...
    */
    ...
    protected function createRenderer(string $format) : VariableRenderer
    {
        ...
        // Gives a result like "String\StringRendererArray"
        $classTypeName = sprintf(
            '%s\%sRenderer%s',
            $format,
            $format,
            $name
        ...
        // Use the base class to dynamically determine the
        // namespace to use for the renderer class.
        $template = str_replace(ClassHelper::getClassTypeName(BaseStringRenderer::class), 'REPLACE', BaseStringRenderer::class);
        $class = str_replace('REPLACE', $classTypeName, $template);
